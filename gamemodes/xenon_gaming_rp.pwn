...
new 	
	PlayerData[MAX_PLAYERS][p_PlayerData],
	Float: SpawnX,
	Float: SpawnY,
	Float: SpawnZ,
	Float: SpawnA,
...
static gSQLConnection;

main() {}

// SQL Functions //
LoadServerSettings() return mysql_tquery(gSQLConnection, "SELECT SpawnX, SpawnY, SpawnZ, SpawnA, SpawnInterior, SpawnVW, ModeName FROM settings", "OnLoadServerSettings", "");

forward OnLoadServerSettings();
public OnLoadServerSettings()
{
    if(cache_num_rows() == 0) return print("[Server]: Failed to load setting row. Reset to default.");
...
{
    if(cache_num_rows() == 0) return ShowPlayerDialog(playerid, PlayerDialogRegister, DIALOG_STYLE_PASSWORD, "Please register.", "Please choose a password and click Register to continue.", "Register", "Logout");
    if(cache_get_field_content_int(0, "AccountDisabled") || cache_get_field_content_int(0, "AccountBanned")) return true;
...
{
    if(cache_affected_rows() == 0) return Kick(playerid);
    ...
    switch(dialogid)
    {
        case PlayerDialogLogin:
        {
            ...
            else
            {
                ...
                if(PlayerData[playerid][pLoginAttempts] >= 3) return Kick(playerid);
        ...
        case PlayerDialogRegister:
        {
            if(!response) return Kick(playerid);  
            if(strlen(inputtext) < 6 || strlen(inputtext) >= 32) return ShowPlayerDialog(playerid, PlayerDialogRegister, DIALOG_STYLE_PASSWORD, "Please register.", "Please choose a password and click Register to continue.\nPassword can not be shorter than 6 characters or longer than 32.", "Register", "Logout");
...
public OnGameModeInit()
{
    ...
    if(mysql_errno(gSQLConnection) == 0) print("[Server]: Database connection successful.");
    else
    {
...
{
    for(new i; i <= GetPlayerPoolSize(); i++) if(IsPlayerConnected(i) && cache_is_valid(PlayerData[i][pLoginCache])) 
...
public OnPlayerConnect(playerid)
{
    ...
    GetPlayerIp(playerid, PlayerData[playerid][pPlayerIP], 16);
